...
/*public*/ void init_neighbors(int num)
{
  for (int row = 0; row < ROW; ++row) {
    for (int col = 0; col < COL; ++col) {
      ...
      int me = row*COL + col;
      collect_neighbors(adjacent, row, col, neighbors[num][me]);

      if (DEBUG_MODE)
        print_neighbors(adjacent, row, col, neighbors[num][me]);
    }
  }
}
...
bool solved(int num)
{
  ...
  for (int row = 0; row < ROW; row += 3) {
    for (int col = 0; col < COL; col += 3) {
      int occurs[10] = { 0 };
      ++occurs[chess[num][row  ][col]];
      ++occurs[chess[num][row  ][col+1]];
      ++occurs[chess[num][row  ][col+2]];
      ++occurs[chess[num][row+1][col]];
      ++occurs[chess[num][row+1][col+1]];
      ++occurs[chess[num][row+1][col+2]];
      ++occurs[chess[num][row+2][col]];
      ++occurs[chess[num][row+2][col+1]];
      ++occurs[chess[num][row+2][col+2]];
